...
$sql = "select round(sum(valor_os)/count(distinct ordem_servico),2) as valor_medio from MANUTENCAO where upper(cliente) = '".$_SESSION['empresa_usr']."' 
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "R$ ".number_format($row['valor_medio'], 2, ',', '.'); //$data[] = $row;
    }
} else {
    echo "ERRO";
}
